...
CONF="$(echo "$PWD" | sed -n "s_^\(.*\)/grupo5_\1_p")/grupo5/conf/AFRAINST.conf"	#la carpeta de configuracion donde saco todas las direcciones
NOVEDIR="NOVEDIR"
RECHDIR="RECHDIR"
REPODIR="REPODIR"
ACEPDIR="ACEPDIR"
...
arrancarAFRAECI(){
	...
	if [ $RETORNO == "No" ]; then
		...
		if ! [ "$(ps aux | grep -c '/bin/bash \.AFRARECI\.sh')" -le 2 ]; then  
