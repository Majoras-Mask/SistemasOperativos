...
minInicializacion(){
	export CONF=$(echo "$PWD" | sed -n 's-^\(.*\)grupo5.*-\1grupo5/conf/AFRAINST.conf-p')
...
loguearVariables(){
	...
	for(( i=0;i<${#listar[@]};i++ )); do
		Loguear "Directorio de ${tipoDir[${i}]}: ${listar[${i}]} $(echo) $(ls "${listar[${i}]}") " "INFO"
	done
...
verificaInstalacionCompleta(){
	...
	local bin=('AFRAINIC.sh' 'AFRARECI.sh' 'AFRAUMBR.sh' 'ARRANCAR.sh' 'DETENER.sh' 'GRALOG.sh' 'MOVERA.sh' 'AFRALIST.pl')
	local cantBin=${#bin[@]}
...
arrancarAFRAECI(){
	...
	if [ $RETORNO == "No" ]; then
		...
		pid=$(ps aux | grep "AFRARECI" | grep -v 'ARRANCAR' | grep -v 'grep' | grep '/bin/bash' | head -n 1 | awk '{print $2}')
...
existenCarpetas(){
	...
	local folder=("$NOVEDIR" "$ACEPDIR" "$PROCDIR" "$REPODIR" "$RECHDIR" "$PROCDIR/proc" "$RECHDIR/llamadas")
	local cant=${#folder[@]}
